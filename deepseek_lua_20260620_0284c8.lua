-- V1PRWARE - Full Optimized Version
...
local keyNameMap = {
    ...
    ["RETURN"] = Enum.KeyCode.Return, ["ESC"] = Enum.KeyCode.Escape,
    ["ESCAPE"] = Enum.KeyCode.Escape, ["TAB"] = Enum.KeyCode.Tab,
    ["CAPS"] = Enum.KeyCode.CapsLock, ["CAPSLOCK"] = Enum.KeyCode.CapsLock,
    ["SHIFT"] = Enum.KeyCode.LeftShift, ["CTRL"] = Enum.KeyCode.LeftControl,
    ["CONTROL"] = Enum.KeyCode.LeftControl, ["ALT"] = Enum.KeyCode.LeftAlt,
    ["DELETE"] = Enum.KeyCode.Delete, ["DEL"] = Enum.KeyCode.Delete,
    ["BACKSPACE"] = Enum.KeyCode.Backspace, ["BACK"] = Enum.KeyCode.Backspace,
    ["INSERT"] = Enum.KeyCode.Insert, ["INS"] = Enum.KeyCode.Insert,
    ["HOME"] = Enum.KeyCode.Home, ["END"] = Enum.KeyCode.End,
    ["PGUP"] = Enum.KeyCode.PageUp, ["PAGEUP"] = Enum.KeyCode.PageUp,
    ["PGDN"] = Enum.KeyCode.PageDown, ["PAGEDOWN"] = Enum.KeyCode.PageDown,
...
local function espCreate(obj, color, labelText, isCharacter, isGenerator, puzzleCount, totalPuzzles)
    pcall(function()
        if not obj or not obj.Parent then return end
        
        -- Check if already exists and is valid
        if esp.highlights[obj] and esp.highlights[obj].Parent then
            return
        end
        
        local attach = obj:FindFirstChild("HumanoidRootPart") or obj:FindFirstChild("Torso") 
            or obj:FindFirstChildWhichIsA("BasePart") or obj:FindFirstChildOfClass("BasePart")
        if not attach then return end
        
        -- Create highlight with unique name
        local hl = Instance.new("Highlight")
        hl.Name = "V1PR_ESP_" .. tostring(obj):gsub("[^%w]", "")
        hl.FillColor = color
        ...
        bb.Adornee = attach
        bb.Size = UDim2.new(0, 240, 0, isGenerator and 30 or 20)
        bb.StudsOffset = Vector3.new(0, isCharacter and 3.5 or (isGenerator and 4 or 3), 0)
        bb.AlwaysOnTop = true
        ...
        label.TextSize = isGenerator and 14 or 10
        ...
        -- Generator puzzle tracking
        if isGenerator then
            local updateGenLabel = function()
                if not obj or not obj.Parent then return end
                local puzzles = esp.genPuzzles[obj] or {}
                local done = 0
                for _, v in pairs(puzzles) do if v then done = done + 1 end end
                local total = totalPuzzles or 4
                local percent = math.floor((done / total) * 100)
                local status = done >= total and "✅ COMPLETE" or string.format("%d/%d (%.0f%%)", done, total, percent)
                if label and label.Parent then
                    local name = obj.Name or "Generator"
                    label.Text = string.format("%s\n%s", name, status)
                    if done >= total then
                        label.TextColor3 = COLORS.GenDone
                        if hl then hl.FillColor = COLORS.GenDone end
                    end
                end
            end
            ...
            -- Watch for puzzle completions
            local genFolder = obj:FindFirstChild("Puzzles")
            if genFolder then
                for _, child in ipairs(genFolder:GetChildren()) do
                    if child:IsA("BoolValue") then
                        table.insert(esp.connections, child:GetPropertyChangedSignal("Value"):Connect(updateGenLabel))
                    end
                end
            end
        end
        
        -- Character update with throttling
        if isCharacter then
            local lastCharUpdate = 0
            local function updateCharLabel()
                if not obj or not obj.Parent or not label or not label.Parent then return end
                local now = tick()
                if now - lastCharUpdate < 0.3 then return end
                lastCharUpdate = now
                ...
                local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
                if myRoot then
                    local a = obj:FindFirstChild("HumanoidRootPart") or obj:FindFirstChild("Torso") 
                        or obj:FindFirstChildWhichIsA("BasePart")
                    if a then 
                        distText = string.format("%.0f", (a.Position - myRoot.Position).Magnitude) 
                    end
                end
            ...
            local hum = obj:FindFirstChildOfClass("Humanoid")
            if hum then 
                esp.healthConnections[obj] = hum.HealthChanged:Connect(updateCharLabel) 
            end
            
            -- Clean up on death
            if hum then
                table.insert(esp.connections, hum.Died:Connect(function()
                    task.wait(0.5)
                    if esp.highlights[obj] then 
                        pcall(function() esp.highlights[obj]:Destroy() end)
                        esp.highlights[obj] = nil
                    end
                    if esp.billboards[obj] then 
                        pcall(function() esp.billboards[obj]:Destroy() end)
        ...
        table.insert(esp.connections, obj.AncestryChanged:Connect(function()
            if not obj.Parent then
                pcall(function()
                    ...
                    esp.generators[obj] = nil
                    esp.genPuzzles[obj] = nil
                end)
            end
        end))
    end)
...
local function espScan()
    pcall(function()
        ...
        if esp.frameCounter % 2 == 0 and now - esp.lastUpdate < esp.updateInterval then
            return
        end
        esp.lastUpdate = now
        ...
        local function addESP(obj, objType, color, label, isGen, puzzles)
            ...
            if myRoot and (attach.Position - myPos).Magnitude > esp.maxDistance then 
                return 
            end
            
            espCreate(obj, color, label, objType == "Character", isGen, puzzles)
        end
            ...
            if kf then 
                for _, m in ipairs(kf:GetChildren()) do 
                    if m:IsA("Model") and m ~= myChar then 
                        ...
                        if h and h.Health > 0 then 
                            addESP(m, "Character", COLORS.Killer, m.Name)
                        end
                    end
                end 
            end
            ...
            if sf then 
                for _, m in ipairs(sf:GetChildren()) do 
                    if m:IsA("Model") and m ~= myChar then 
                        local h = m:FindFirstChildOfClass("Humanoid")
                        if h and h.Health > 0 then 
                            addESP(m, "Character", COLORS.Survivor, m.Name)
                        end
                    end
                end 
            end
        ...
        -- Scan Generators with puzzle tracking
            ...
            if mc then 
                for _, o in ipairs(mc:GetChildren()) do 
                    if o.Name == "Generator" then
                        ...
                        local puzzles = {}
                        local genFolder = o:FindFirstChild("Puzzles")
                        if genFolder then
                            for _, child in ipairs(genFolder:GetChildren()) do
                                if child:IsA("BoolValue") then
                                    totalPuzzles = totalPuzzles + 1
                                    puzzles[child] = child.Value
                                end
                            end
                        end
                        esp.genPuzzles[o] = puzzles
                        addESP(o, "Object", COLORS.Generator, "Generator", true)
                    end
                end 
            end
        end
        
        -- Scan Items (throttled)
        if esp.showItems and esp.frameCounter % 3 == 0 then
            for _, o in ipairs(svc.WS:GetDescendants()) do 
                if (o.Name == "BloxyCola" or o.Name == "Medkit") then
                    addESP(o, "Object", COLORS.Item, o.Name)
                end
            end
        end
        
        -- Scan Structures (throttled)
        if esp.showStructures and esp.frameCounter % 3 == 0 then
            local ig = espGetIngame()
            if ig then 
                for _, o in ipairs(ig:GetChildren()) do 
                    if o.Name == "BuildermanSentry" or o.Name == "SubspaceTripmine" or o.Name == "BuildermanDispenser" then
                        addESP(o, "Object", COLORS.Structure, o.Name)
                    end
                end 
            end
        end
    ...
    pcall(function()
        ...
        local function onTeamChild(child, team, color)
            ...
            if esp.enabled and child:IsA("Model") and child ~= lp.Character then
                ...
                if h and h.Health > 0 and esp["show"..team] then 
                    espCreate(child, color, child.Name, true) 
                end
            end
        end
        ...
        local kf = espGetTeamFolder("Killers")
        if kf then 
            table.insert(esp.connections, kf.ChildAdded:Connect(function(c) 
                onTeamChild(c, "Killers", COLORS.Killer) 
            end)) 
        end
        
        local sf = espGetTeamFolder("Survivors")
        if sf then 
            table.insert(esp.connections, sf.ChildAdded:Connect(function(c) 
                onTeamChild(c, "Survivors", COLORS.Survivor) 
            end)) 
        end
        ...
        table.insert(esp.connections, svc.WS.DescendantAdded:Connect(function(o)
            task.wait(0.1)
            if esp.enabled then
                if o.Name == "Generator" and esp.showGenerators then
                    local puzzles = {}
                    local genFolder = o:FindFirstChild("Puzzles")
                    if genFolder then
                        for _, child in ipairs(genFolder:GetChildren()) do
                    ...
                    esp.genPuzzles[o] = puzzles
                    espCreate(o, COLORS.Generator, "Generator", false, true)
                elseif (o.Name == "BloxyCola" or o.Name == "Medkit") and esp.showItems then
                    espCreate(o, COLORS.Item, o.Name)
                elseif esp.showStructures and (o.Name == "BuildermanSentry" or o.Name == "SubspaceTripmine" or o.Name == "BuildermanDispenser") then
                    espCreate(o, COLORS.Structure, o.Name)
                end
            end
        end))
    end)
...
local function isRealPlayer(obj)
    for _, p in ipairs(svc.Players:GetPlayers()) do 
        if p.Character == obj or (p.Character and obj:IsDescendantOf(p.Character)) then 
            return true 
        end 
    end
    return false
    ...
    pcall(function()
        for tag, tbl in pairs(tracked) do 
            for obj in pairs(tbl) do 
                local h=obj:FindFirstChild(tag.."_HL")
                if h then h.FillTransparency=mset.transparency end 
            end 
        end
    end)
end
...
local function clearTag(tag)
    pcall(function()
        for obj in pairs(tracked[tag]) do
            ...
            local b=obj:FindFirstChild(tag.."_BB")
            if b then b:Destroy() end
            if tag=="puddle" then 
                local h2=obj:FindFirstChild("PuddleHolder")
                if h2 then h2:Destroy() end 
            end
        end
        tracked[tag]={}
...
local function addPuddleHighlight(part, color, tag, label)
    pcall(function()
        ...
        local bc = Instance.new("CylinderHandleAdornment")
        bc.Name="PuddleBlack"
        bc.Adornee=holder
        bc.Color3=Color3.fromRGB(0,0,0)
        bc.Transparency=0.2
        bc.Radius=radius
        bc.Height=0.02
        bc.CFrame=CFrame.Angles(math.rad(90),0,0)
        bc.ZIndex=5
        bc.AlwaysOnTop=true
        bc.Parent=holder
        
        local ro = Instance.new("CylinderHandleAdornment")
        ro.Name="PuddleRed"
        ro.Adornee=holder
        ro.Color3=Color3.fromRGB(255,0,0)
        ro.Transparency=0.4
        ro.Radius=radius+0.8
        ro.Height=0.02
        ro.CFrame=CFrame.Angles(math.rad(90),0,0)
        ro.ZIndex=4
        ro.AlwaysOnTop=true
        ro.Parent=holder
...
local function isJohnDoePuddle(o)
    ...
    local p = o.Parent
    return p and p.Name:find("Shadows$") ~= nil
end
...
local function scanPizza() 
    pcall(function() 
        if mset.pizza then 
            for _,o in ipairs(svc.WS:GetDescendants()) do 
                if o.Name=="PizzaDeliveryRig" and o:IsA("Model") and not isRealPlayer(o) and not tracked.pizza[o] then 
                    addHighlight(o,Color3.fromRGB(255,100,0),"pizza","C00LKIDD PIZZA DELIVERY",3) 
                end 
            end 
        end 
    end) 
...
local function scanZombie() 
    pcall(function() 
        if mset.zombie then 
            for _,o in ipairs(svc.WS:GetDescendants()) do 
                if o.Name=="1x1x1x1Zombie" and o:IsA("Model") and not isRealPlayer(o) and not tracked.zombie[o] then 
                    addHighlight(o,Color3.fromRGB(80,255,120),"zombie","1X1X1X1 ZOMBIE",3) 
                end 
            end 
        end 
    end) 
...
local function scanPuddles() 
    pcall(function() 
        if mset.puddle then 
            for _,o in ipairs(svc.WS:GetDescendants()) do 
                if isJohnDoePuddle(o) and not tracked.puddle[o] then 
                    addPuddleHighlight(o,Color3.fromRGB(255,50,50),"puddle","JOHN DOE PUDDLE") 
                end 
            end 
        end 
    end) 
...
secMinion:Toggle({ Title="c00lkidd Pizza Bots", Type="Checkbox", Flag="espPizza", Default=mset.pizza, 
    Callback=function(on) pcall(function() mset.pizza=on; if on then scanPizza() else clearTag("pizza") end end) end })

secMinion:Toggle({ Title="1x1x1x1 Zombies", Type="Checkbox", Flag="espZombie", Default=mset.zombie, 
    Callback=function(on) pcall(function() mset.zombie=on; if on then scanZombie() else clearTag("zombie") end end) end })

secMinion:Toggle({ Title="JD Digital Footprints", Type="Checkbox", Flag="espPuddle", Default=mset.puddle, 
    Callback=function(on) pcall(function() mset.puddle=on; if on then scanPuddles() else clearTag("puddle") end end) end })

secMinion:Slider({ Title="Highlight Transparency", Flag="espMinionTrans", Step=0.05, Value={Min=0,Max=1,Default=mset.transparency}, 
    Callback=function(v) pcall(function() mset.transparency=v; updateTransparency() end) end })
